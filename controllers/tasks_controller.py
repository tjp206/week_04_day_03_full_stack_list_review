...
@tasks_blueprint.route("/tasks") # setting a route
def tasks():
    tasks = task_repository.select_all()
    return render_template("tasks/index.html", all_tasks=tasks)

@tasks_blueprint.route("/tasks/new") # add new task
def new_task():
...
@tasks_blueprint.route("/tasks/<id>/delete", methods=["POST"]) # delete task from list
def delete(id):
